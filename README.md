...
`React 18` · `TypeScript` · `Fastify` · `Prisma` · `SSE` · `PWA` · **테스트 220개 통과** · `MIT`
...
### C. 실사용 실측 — 실제 키 3개로 전 구간 (2026-07-28)

`npm run measure:keys`(프론트엔드)가 **실제 사용자 키 3개**로 브라우저 3개를 띄워 전 구간을 돌리고, 제공자가 돌려주는 **자체 사용량 집계**(`usageMetadata`)를 기록합니다. 목킹 없음 — 진짜 과금되는 호출입니다.

**시나리오**: 게스트 3명(아라·바다·찬) → 커뮤니티 생성 → 게시글 + 1차 AI 답변 → **9턴 다자 토론(그중 4턴이 `@AI`)** → `[ 문서로 정리 ]` → 대조군/첨부 스레드에서 **같은 질문**을 문서 없이/있이 각 1회.

| 호출 종류 | 횟수 | 평균 prompt | 평균 total | 비용(USD) |
|---|---|---|---|---|
| 1차 AI 답변 (FR-4.3) | 1 | 118 | 292 | $0.000291 |
| `@AI` 답변 (FR-6) | 4 | 766 | 952 | $0.001880 |
| **문서 응결 (FR-13)** | 1 | 1,493 | 2,211 | $0.001450 |
| 문서 첨부 `@AI` (FR-14) | 1 | 923 | 1,099 | $0.000495 |
| 대조군(첨부 없음) | 1 | 129 | 305 | $0.000296 |

| 지표 | 실측값 |
|---|---|
| 참여자 / 스레드 / 응결 문서 | 3명 / 3개 / **1건**(렌더 1,267자, 출처 `seq 15`) |
| 전체 LLM 호출 | **8회**, 총 **7,713 토큰** |
| **사용자 3명 합계 비용** | **$0.004412** (1인 평균 $0.0015) |
| **운영자 LLM 비용** | **$0.00** — 서버에 제공자 호출 경로가 존재하지 않음 |
| **키 유출 검사** | Aidit 서버 요청 **327건 중 0건** (바디·쿼리·헤더 전수 검사) ✅ |
| 공유 컨텍스트 성장 | `@AI` 호출별 prompt **399 → 650 → 898 → 1,116** 토큰 |
| FR-14 첨부 순증 | 같은 질문에서 prompt **129 → 923** (**+794 토큰**, +$0.000199) |

읽는 법 세 가지:

1. **BYOK가 실제로 작동합니다.** 3명이 각자 자기 키로 지불했고(아라 $0.00219 / 바다 $0.00175 / 찬 $0.00048), **서버 청구액은 0**입니다. key-blind도 주장이 아니라 라이브 트래픽 327건 전수 검사로 확인했습니다.
2. **공유 컨텍스트의 비용 곡선이 보입니다.** 같은 스레드의 `@AI` 호출은 399→1,116 토큰으로 커집니다 — 이 증가를 영원히 방치하지 않기 위해 128K 임계 요약(FR-7)이 있고, 그 트리거 자체도 다음 호출자의 키로 지불됩니다.
3. **문서 응결이 가장 비싼 단일 동작**(2,211 토큰)이지만 **한 스레드에 한 번**이고, 그 산출물이 재사용됩니다. FR-14 첨부는 질문당 +794 토큰(+$0.0002)에 "커뮤니티가 이미 합의한 내용"을 얻는 거래입니다.

> **모델·단가**: `gemini-3.1-flash-lite`, 유료 티어 입력 $0.25 / 출력 $1.50 per 1M([공식 단가](https://ai.google.dev/gemini-api/docs/pricing), 2026-07-28 확인). 이 모델에는 **무료 티어도 있어 실제 사용자는 $0**일 수 있으며, 위 금액은 "전부 과금됐다면"의 상한입니다.
> **한계**: 각본화된 1회 세션이며 유기적 사용 통계가 아닙니다. 재현: `cd frontend && DEMO_KEY_A=… DEMO_KEY_B=… DEMO_KEY_C=… AIDIT_BASE=http://localhost:5173 npm run measure:keys`

### D. 토큰 추정 정확도 — 128K 판정의 근거 (2026-07-28)

128K 자동 요약(FR-7)은 **로컬 추정 토큰의 누적 합**으로 판정합니다. 발화마다 제공자 `countTokens`를 부르면 왕복이 추가되고 사용자 키의 분당 한도를 생성 호출과 나눠 쓰게 되므로, **`countTokens`는 오프라인 보정에만** 써서 계수를 실측했습니다.

| 지표 (앱 형태 10개 실측 샘플) | 기존 `chars/4` | **보정식 `ceil(dense/1.3 + rest/4.5)`** |
|---|---|---|
| 누적 오차 | **−39.4%** | **+7.9%** (과대 = 안전) |
| 최악 단일 과소평가 | −58% | **−3%** |
| 평균 절대 오차 | 44% | **9%** |

`chars/4`는 라틴 기준 경험식이라 **한국어를 최대 58% 과소평가**했고, 그 결과 128K 임계가 한참 늦게 걸려 정책이 사실상 무력화된 상태였습니다 — 정확도 문제가 아니라 결함이었습니다.

**보정에서 한 번 틀렸고 그 기록을 남겼습니다.** 처음엔 이 리포의 문서를 코퍼스로 썼는데, 추정기가 실제로 보는 건 문서가 아니라 **댓글·AI 답변·응결 문서**입니다. 대화체 한국어와 기술 문서의 토큰 밀도가 1.5배 다르기 때문에, 문서 기준 계수는 실제 콘텐츠를 +34% 과대평가했습니다. 앱 형태 샘플로 다시 적합한 것이 위 값입니다. 남는 편향은 **의도적으로 과대 방향**입니다 — 과소평가는 정책을 조용히 넘기지만, 과대평가는 요약이 조금 일찍 실행될 뿐입니다.

재보정(모델 교체 시 필수): `cd backend && CAL_KEY=<키> npm run calibrate:tokens` — 상세는 [TRD §6.4](./docs/TRD.md)(계수의 SoT).

> **`countTokens` 비용에 대한 정확한 서술**(2026-07-28 확인): "There's no charge for calling `countTokens`", 쿼터 최대 3,000 RPM — 다만 이 문장의 출처는 [Vertex AI / Firebase AI Logic 문서](https://firebase.google.com/docs/vertex-ai/count-tokens)이고, 이 앱이 실제로 호출하는 **Gemini Developer API**(`generativelanguage.googleapis.com`) 문서에는 과금 여부가 **명시돼 있지 않습니다**. 또한 과금이 없어도 **RPM 쿼터는 소비**합니다. 초기 서술에서 이를 그냥 "무료"라고 단정했는데, 근거 범위를 위와 같이 좁혔습니다. 런타임에서 `countTokens`를 부르는 곳은 **키당 1회 연결 프로브**(FR-8)뿐이며, 발화별 토큰 산정에는 쓰지 않습니다.
...
| 항목 | 상태 |
|---|---|
| 백엔드 테스트 | **148 통과** (계약 · SSE · 문서 응결 · pub/sub fan-out · 레이트리밋 저장소 · CORS 정책 · 토큰 추정 보정 · **이벤트 싱크** · hotScore · 프로필 페이지네이션) |
| 프론트엔드 테스트 | **81 통과** (컨텍스트 엔진 · 문서 엔진 · 문서 컨텍스트 스토어 · sanitize · LLM 클라이언트 · **토큰 추정 동기**) |
| 타입체크 | 양쪽 `tsc --noEmit` 클린 |
| E2E | Playwright **5스펙 10케이스 전부 통과** — 콜드 스타트에서 `npm run e2e`가 백엔드+Vite를 **스스로 띄워** J1~J3 · J4 응결 4 · J5 재투입 2를 통과하고, 실키 BYOK 스펙도 **실제 키로 통과**(2026-07-28 실측) |
| 관측성 | **브라우저 전용 KPI가 서버에 실제로 남습니다** — 이벤트 싱크(TRD §16) 도입 후 `GET /metrics`가 `llmSuccessRate`를 실측 반환(라이브 확인: J1~J3 + 실키 호출까지 `llm_success` 5건 집계, 이전에 동작마다 발생하던 `/metrics/events` 404 **0건**). 남은 공백은 `p95PropagationMs` 하나 |
...
| CI | **`deploy/pipeline.sh`로 리포에 코드화** — 기본 5게이트(양쪽 typecheck·test + Postgres 스키마 드리프트)와 `--all` **8게이트(+E2E 자체 기동 +프로덕션 빌드) 전부 통과**, 게이트 실패 시 `exit 1`까지 실측 확인. GitHub Actions는 쓰지 않습니다(의도된 선택, [배포 · CI](#-배포--ci)) |
