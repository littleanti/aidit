...
- **이벤트 카운터(`EventCounter`)**: 브라우저만 관측 가능한 이벤트의 **일별 집계**(§16). 원본 이벤트 행을 쌓지 않는 이유는 두 가지다 — ① `props`를 저장하지 않으므로 개별 행에 남을 정보가 없고, ② 카운터는 `@@unique([name, date])` + `increment`로 **다중 인스턴스에서도 DB가 단일 출처**가 된다(인메모리 카운터는 인스턴스마다 갈리고 재시작에 사라진다).
...
| **`POST /metrics/events`** | **브라우저 전용 이벤트 싱크**(§16) — 허용목록 이벤트를 일별 카운터로 증가 | - (비인증) | **`props`는 읽지도, 저장하지도, 로그하지도 않는다**(key-blind 방어) · 미허용 이벤트는 202 `{counted:false}` · 레이트리밋 120/분 |
| `GET /metrics` | §8 KPI 집계 반환 | - | BE-13 (형상: IMPLEMENTATION_NOTES §2.2) · `llmSuccessRate`는 §16 카운터에서 산출 |
...
```
tokens ≈ ceil( dense / 1.3 + rest / 4.5 )
```

- `dense` = 한글(및 CJK·가나) 문자 수, `rest` = 그 외 문자 수(라틴·숫자·기호·마크다운 문법).
- **왜 바꿨나**: 기존 `ceil(chars/4)`는 라틴 기준 경험식이다. 실측(모델 `gemini-3.1-flash-lite`)에서 한국어는 **1토큰 ≈ 1.7~1.9자**, 영어는 **1토큰 ≈ 4.5~5.2자**였다. 그 결과 `chars/4`는 앱 콘텐츠 **누적 합을 −39.4%**(최악 단일 **−58%**) 과소평가했다 → 128K 임계가 한참 늦게 걸려 FR-7 정책이 사실상 무력화된 상태였다. **정확도 문제가 아니라 결함이다.**

**보정 과정에서 한 번 틀렸고, 그 기록을 남긴다 — "무엇으로 보정하는가"가 계수보다 중요하다**

1차 보정은 **이 리포의 문서(PRD·TRD·README·코드)** 를 코퍼스로 썼고 `dense/0.9`가 나왔다. 그런데 추정기가 실제로 보는 텍스트는 문서가 아니라 **댓글·AI 답변·응결 문서**다. 측정해 보니 두 계열의 한국어 토큰 밀도가 다르다:

| 콘텐츠 유형 | dense 문자당 토큰 | 앱이 이걸 처리하는가 |
|---|---|---|
| 대화체 한국어(댓글·AI 답변) | ~0.65 | **예** |
| 기술 한국어 + 마크업(스펙 문서) | ~1.0 | 아니오(리포 문서) |

문서 기준 계수를 앱 콘텐츠에 적용하면 **+34% 과대평가**가 났다. 그래서 **앱 형태 샘플만으로 다시 적합**했다.

| 지표 (앱 형태 10개 실측 샘플) | `chars/4` | `dense + rest/4`(1차 오류) | **채택식 `dense/1.3 + rest/4.5`** |
|---|---|---|---|
| 누적 오차 | **−39.4%** | +34.3% | **+7.9%** |
| 최악 단일 과소평가 | −58% | 0% | **−3%** |
| 최악 단일 과대평가 | +30% | +47% | +22% |
...
- **선택 근거**: 128K 판정은 개별 발화가 아니라 **누적 합**에 대한 것이므로 총합 오차를 0 근처로 맞추되, 남는 편향은 **과대 방향**이어야 한다. 과소평가는 정책을 조용히 넘기지만(컨텍스트가 의도보다 커진다), 과대평가는 요약이 조금 일찍 실행될 뿐이다. 더 정확한 후보(`dense/1.4`, 누적 +1.0%)도 있었지만 최악 과소평가가 −13%로 커져 채택하지 않았다.
- 128K는 **모델 한도가 아니라 제품 정책값**(A-2/FR-7)이므로 어느 방향도 하드 실패가 아니다.
- **구현 동기화 주의**: 같은 상수가 **두 곳**에 있다 — `frontend/src/api/llm.ts`(`estimateTokens`)와 `backend/src/domain/tokenEstimate.ts`(서버가 클라 제출값 없이 산정할 때). 이 절이 두 구현의 SoT이며, 한쪽만 바꾸면 서버 `tokenSum`과 클라 판단이 갈린다. 프론트 테스트에 **두 공식의 동기 검증**이 있다.
- **재보정 방법**: `cd backend && CAL_KEY=<키> npm run calibrate:tokens`. 앱 형태 샘플에 `countTokens`를 1회씩 호출해 현재 추정기의 누적 오차와 최소제곱 적합값을 출력한다. **모델을 바꾸면 반드시 재보정한다**(토크나이저가 다르다). 누적 오차가 음수로 넘어가면 즉시 손봐야 한다 — 그게 위험한 방향이다.
...
- ~~토큰 카운팅 정확도 vs `countTokens` 호출 비용/지연 트레이드오프~~ → **해결(2026-07-28)**: 런타임 `countTokens` 호출은 채택하지 아니하고(발화마다 왕복 추가), **오프라인 보정**으로 해결했다. 실측 결과 기존 `chars/4`가 한국어를 최대 −58% 과소평가하여 128K 정책이 무력화되고 있었고, 스크립트 인식 추정식 `ceil(dense/1.3 + rest/4.5)`로 교체해 앱 콘텐츠 누적 오차를 −39.4% → **+7.9%**(과대 = 안전 방향), 평균 절대 오차를 44% → **9%** 로 줄였다. 상수의 SoT는 §6.4이며 재보정은 `npm run calibrate:tokens`.
- 요약 품질 평가 루프(요약이 사실 보존하는지) — 후속 자동평가. **참고**: FR-13 문서 응결은 같은 성격의 산출물이며, 실키 실측에서 생성된 문서가 토론에서 실제로 갈렸던 쟁점을 "미해결 질문"으로 도출하는 것까지 사람 눈으로 확인했다(자동평가는 여전히 미구현).
...
> 이 절은 **서버가 key-blind이기 때문에 생기는 관측 공백**을 어떻게 메우는지를 정한다.

### 16.1 문제 — 판단이 브라우저에서 일어난다

LLM 호출은 브라우저가 사용자 키로 직접 한다(§5). 따라서 **성공/실패·지연·요약 발동 같은 제품의 핵심 사건이 서버 로그에 아예 존재하지 않는다.** `GET /metrics`가 `llmSuccessRate: null`을 반환하며 `unavailable`에 이유를 적어 둔 것이 그 공백의 표현이었다.

클라이언트에는 이미 계측이 있었다(`frontend/src/lib/metrics.ts → track()`, 호출 지점 16곳). 그런데 **받는 쪽이 없어서** 모든 이벤트가 `POST /api/metrics/events` 404로 사라졌다. "계측은 했지만 아무것도 남지 않는" 상태였고, 라이브 실행 로그에서 이 404가 그대로 관측됐다.

### 16.2 이벤트 싱크 — 카운터이지 로그가 아니다

`POST /metrics/events`는 **허용목록에 있는 이벤트 이름만** 받아 `EventCounter(name, date)`를 1 증가시킨다.

- **`props`는 읽지도, 저장하지도, 로그하지도 않는다.** 이것이 이 설계의 핵심 제약이다. 비인증 엔드포인트에 임의 JSON을 로그로 흘리면, 클라이언트 실수 하나로 **사용자 키가 서버 로그에 착륙**할 수 있다 — key-blind(L1)가 무너지는 경로다. 그래서 싱크는 **로그가 아니라 카운터**다. 디버깅용 상세는 브라우저 `console.debug`에 남고 거기서 끝난다.
- **비인증**: 계측은 로그인 전(`login` 실패 등)에도 발생하고, 인증을 요구하면 그 구간이 통째로 사라진다. 대신 **레이트리밋 120/분**(IP 키잉)으로 남용을 막는다.
- **미허용 이벤트는 400이 아니라 202 `{counted:false}`**. 텔레메트리가 사용자 흐름을 깨서는 안 되고, 프론트가 이벤트를 추가했을 때 서버 배포 전까지 4xx 소음이 나는 것도 바람직하지 않다. 다만 **조용히 세지도 않는다** — 응답에 사실을 적어 되돌려준다.
- 허용목록은 `track()` 호출 지점과 1:1이며, 새 이벤트를 세려면 **양쪽을 같이 고쳐야 한다**(의도된 마찰: 무엇을 세는지가 계약이다).

### 16.3 이걸로 닫히는 것과 남는 것

| KPI | 상태 |
|---|---|
| `llmSuccessRate` (§8 목표 ≥0.97) | **닫힘** — `llm_success / (llm_success + llm_failure)` 일별 카운터에서 산출 |
| 요약·문서 응결 성공률 | **닫힘** — `summary_*`, `document_*` 카운터로 산출 가능 |
| `p95PropagationMs` (§8 목표 <1500) | **남음** — SSE 전파 지연은 종단 측정이 필요하고, 카운터로는 분포를 알 수 없다. 히스토그램 버킷을 이벤트로 보내는 방식이 후속 |
| 서버측 요청 추적 | **부분** — Fastify가 `reqId`와 요청/응답 로그를 이미 남긴다. 요청 ID를 클라이언트 오류 리포트와 상관시키는 것은 후속 |

- **원본 이벤트를 남기지 않는 대가**: 세션 단위 분석이나 퍼널 추적은 불가능하다. PoC의 §8 KPI는 전부 비율·평균이므로 카운터로 충분하고, 그 이상이 필요해지는 시점은 **개인정보 판단이 함께 필요한 시점**이라 지금 앞당기지 않는다.
