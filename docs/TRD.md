...
- 토큰 카운팅 정확도 vs `countTokens` 호출 비용/지연 트레이드오프. **2026-07-28 갱신**: 128K 판정은 여전히 클라이언트 추정치(`estimateTokens` = chars/4)를 서버 `tokenSum`에 누적한 값을 쓴다. 다만 실키 실측(README "성능 실측 C")으로 **제공자의 실제 `promptTokenCount`** 를 얻었으므로(같은 스레드에서 399→650→898→1,116), 추정치와 실측치를 대조해 계수를 보정하거나 `countTokens`로 교체하는 판단이 이제 **측정 가능한 후속 작업**이 되었다.
