...
>
> **2026-06-19 로고 기반 리브랜딩 (v0.4)** — Aidit 로고(블루→바이올렛 그라디언트 "A" 마크)를
> 단일 출처로 비주얼 아이덴티티를 재정렬한다. 핵심 변경: ① **브랜드 컬러 바이올렛(#7c3aed) →
> 로고 코어 인디고-바이올렛(#6848F8) + 블루→바이올렛 그라디언트**, ② 헤더·로그인에 **로고 락업**
> (마크+워드마크), ③ 그라디언트는 **시그니처 한정**(전송 버튼·로그인 CTA·로고·AI 아바타),
> ④ 파비콘/앱 아이콘 전면 교체. **색·타이포·로고 사양의 단일 출처는 [DESIGN-SYSTEM.md](./DESIGN-SYSTEM.md)**이며,
> 위 v0.3 §6.3의 플랫 바이올렛 토큰은 이 문서로 대체된다.
