...
## 14. 커뮤니티 문서를 컨텍스트로 첨부 (FR-14) — Composer AI 메뉴 (2026-07-28)

FR-13이 논의를 문서로 내보내는 방향이라면 FR-14는 되돌려 넣는 방향이다. 진입점은 **이미 발화별 AI 설정이 모여 있는 Composer AI 팝오버**다(새 화면을 만들지 않는다).

```
┌──────────────────────────────┐
│ AI 모드            [ ON  ]   │
│ 길이   [짧게][보통][길게]      │
│ 내 페르소나 [없음][반론][요약]  │ ← FR-12
│ ────────────────────────────  │
│ 참고 문서  2/3                │ ← FR-14 (신규)
│  [✓] Code Agent 사용 가이드    │
│  [✓] 리뷰 체크리스트 v2        │
...
- **행 제목**은 `참고 문서 {선택}/3`(FR-14.3 상한). 선택은 **발화 단위·세션 한정**이라 스레드를 떠나면 사라진다(FR-14.2).
- 목록은 팝오버를 **처음 열 때** `GET /communities/:slug/documents`로 지연 로드한다. 최신순 상위 항목만 노출하고, 각 항목은 제목 한 줄(말줄임) + 체크 표시.
- **문서가 없으면** 행 대신 힌트: "이 커뮤니티에 응결된 문서가 없어요 — 스레드 메뉴의 [ 문서로 정리 ]로 만들 수 있어요".
- 상한 도달 시 미선택 항목은 **disabled**(터치해도 무반응)로 두고, 제목의 `3/3`이 이유를 설명한다.
- 첨부가 1개 이상이면 입력창 위에 **`📎 문서 2개 참고` 칩**을 띄워, 팝오버를 닫은 뒤에도 이번 발화에 문서가 붙는다는 사실이 보이게 한다. 칩의 `×`로 전체 해제.
- **불변**: 첨부는 사람 댓글 게시 경로·SSE·`seq`에 영향이 없다. AI 답변 요청에만 참고 턴이 앞에 붙는다(FR-14.6).
