...
> **2026-07-28 갱신 — 16턴은 이제 대안이 있다.** 이 프롬프트로 문서를 "만들어 달라고 부탁"하던 흐름은 **정식 기능으로 승격**됐다: 스레드 `⋯` 메뉴의 **`[ 문서로 정리 ]`**(FR-13)가 같은 일을 버튼 하나로 하고, 결과가 **커뮤니티 문서 탭에 자산으로 누적**되며 출처(세그먼트·턴 수)까지 남는다. 이어서 **FR-14**로 그 문서를 다음 스레드의 참고 자료로 첨부할 수 있다.
>
> 촬영 권장 구성: 16턴을 **`[ 문서로 정리 ]` 실행**으로 대체하고(문서 화면 `/d/:id`까지 이동), 엔딩에 **새 스레드에서 그 문서를 첨부해 질문**하는 컷을 추가한다 — "논의 → 문서 → 다음 논의" 루프가 한 흐름으로 보인다. 기존 프롬프트 방식도 그대로 동작하므로 아래 대본은 유효하다.
