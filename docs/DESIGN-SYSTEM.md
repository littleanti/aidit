...
| **`ShellPrompt` — 라이브 입력 반영 (Live-Prompt)** | 입력 화면에서는 프롬프트 명령이 사용자의 현재 입력을 **실시간으로 반영**할 수 있다. "과하지 않게(tasteful restraint)" 원칙: 레이아웃 이동·가로 스크롤이 없어야 하고, 빈 필드는 항상 기본 명령으로 되돌아가며, 매달린 빈 따옴표는 절대 출력하지 않는다. 명령 자체는 번역하지 않으며, 사용자 입력 인자만 보간한다. 검색 화면은 이미 검증된 선례(`grep -ri "<query>"`); 아래는 나머지 화면의 스펙이다. <br><br>**화면별 라이브 명령 매핑:** <br>• **검색** → `grep -ri "<query>"` — 검색어 입력과 함께 실시간 반영 (이미 구현, 이 스펙에서 불변) <br>• **커뮤니티 생성** → 파생 슬러그가 비어 있으면 `mkdir /c/new` (기본), 슬러그가 있으면 `mkdir /c/<slug>` — 슬러그는 소문자·하이픈 구성이라 셸 안전; 길이 초과 시 잘라냄 (max 30) <br>• **글 작성** → 제목이 비어 있으면 `post --new [r/<slug>]` (기본), 제목이 있으면 `post --new [r/<slug>] "<title>"` — 제목만 반영하며 본문은 절대 노출하지 않음 <br>• **스레드(댓글 작성)** → 댓글 창의 AI-요청 여부(`aiMode` ON 또는 `@AI` 멘션 감지)에 따라 `ai --ask /p/<id8>` 또는 `tail -f /p/<id8>`으로 전환; 댓글 원문은 **의도적으로 반영하지 않음** (키 입력마다 스레드 전체 재렌더가 발생하므로) <br><br>**`formatPromptArg` 공유 유틸 (`src/lib/shellArg.ts`):** 프롬프트 인자 포맷은 순수 함수 `formatPromptArg(raw, opts?)` 하나로 통일한다. 처리 순서: ① 공백·개행 연속을 단일 스페이스로 치환 → ② 앞뒤 trim → ③ 결과가 `max`(기본 32)를 초과하면 `max-1`자로 자르고 말줄임표(`…`) 1개 추가 → ④ 큰따옴표를 `\"`로 이스케이프(장식 목적; 명령은 실행되지 않음). 반환값은 내부 문자열만이며, 따옴표 래핑은 호출처에서 담당한다. <br><br>**A11y 규칙:** 프롬프트 줄 전체(루트 컨테이너)에 `aria-hidden="true"`를 적용한다 — 커서 `<span>`이 아닌 루트 요소에. `aria-live`는 어디에도 추가하지 않는다. 실제 입력 필드가 보조 기술의 접근성 소스이며, 프롬프트는 순수 장식이다. 시각적 렌더링은 이 규칙의 변경 전후로 픽셀 동일해야 한다. |
