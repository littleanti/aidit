...
> 상태: M1–M19 구현 완료 · 최초 작성 2026-06-17 · 최종 수정 2026-07-28 (이벤트 싱크 신설 — 관측 공백 `llmSuccessRate` 닫힘)
...
### 2026-07-28 (11)
- **[feat]** **이벤트 싱크 신설 — 클라이언트 계측이 404로 사라지던 관측 공백을 닫음 (TRD §16)**: 프론트에는 `track()` 호출이 16곳 있었지만 서버에 받는 라우트가 없어 **모든 이벤트가 `POST /api/metrics/events` 404로 버려지고 있었다**(실키 라이브 실행 로그에서 관측). "계측은 했는데 아무것도 남지 않는" 상태였고, `GET /metrics`의 `llmSuccessRate`가 `null`이었던 이유도 이것이다.
  - **로그가 아니라 카운터로 설계**: 허용목록에 있는 **이벤트 이름만** 받아 `EventCounter(name, date)`를 원자적으로 증가시킨다. **`props`는 읽지도·저장하지도·로그하지도 않는다.** 비인증 엔드포인트에 임의 JSON을 로그로 흘리면 클라이언트 실수 하나로 **사용자 키가 서버 로그에 착륙**할 수 있고, 그건 key-blind(L1)가 텔레메트리 때문에 무너지는 경로다. 상세 디버깅 정보는 브라우저 `console.debug`에서 끝난다.
  - **비인증 + 레이트리밋 120/분**: `login` 같은 이벤트는 토큰이 생기기 전에 발생하므로 인증을 요구하면 가장 보고 싶은 구간이 사라진다. 대신 IP 키잉 레이트리밋으로 남용을 막았고, 한도는 넉넉하게 뒀다 — 429로 이벤트를 잃으면 **그 카운터로 계산하는 비율 자체가 편향된다**.
  - **미허용 이벤트는 400이 아니라 202 `{counted:false}`**: 텔레메트리가 사용자 흐름을 깨서는 안 되고, 프론트가 서버보다 먼저 이벤트를 추가했을 때 4xx 소음이 나서도 안 된다. 그렇다고 조용히 버리지도 않고 응답에 사실을 적는다.
  - **다중 인스턴스 정합**: 인메모리 카운터는 인스턴스마다 갈리고 재시작에 사라지므로 DB 카운터 행(`@@unique([name, date])` + `increment`)으로 두어 DB가 단일 출처가 되게 했다. 마이그레이션 `20260728000000_add_event_counter` + Postgres 파생 스키마·`init.sql` 재생성까지 동기화.
  - **닫힌 KPI**: `GET /metrics`의 `llmSuccessRate`가 `llm_success / (llm_success + llm_failure)`로 실제 값을 반환한다(시도 0건이면 `0%`가 아니라 `null` — "데이터 없음"과 "전부 실패"는 달라야 한다). `eventCounts` 원값도 함께 노출해 요약·문서 성공률을 파생할 수 있다. **남은 공백은 `p95PropagationMs`** 하나이며, 카운터로는 분포를 표현할 수 없다는 이유를 `unavailable`에 명시했다(TRD §16.3).
  - **테스트**: `backend/test/metricsEvents.test.ts` **9건** — 허용목록 카운트/미허용 비카운트, 같은 행 증가(다중 인스턴스), 잘못된 바디 관용, **props 미보존(키 문자열이 응답·DB 어디에도 남지 않음 + 저장 컬럼이 4개뿐임을 단정)**, 비인증 허용, `llmSuccessRate` 무데이터 `null`·비율 산출·`unavailable` 목록 변화. 백엔드 **139 → 148 통과**.
  - **라이브 검증**: 스택을 띄워 J1~J3 실행 후 `GET /metrics`가 `llmSuccessRate: 1`, `eventCounts {ai_reply_invoked:3, llm_success:3, login:3, summary_success:1}`을 반환했고, **실키 스펙 실행 후 실제 LLM 호출까지 카운트되어 5로 증가**했다. 서버 로그의 `/metrics/events not found` **0건**(이전엔 동작마다 발생).
...
- **[fix]** **E2E 실행 경로 복구 — J1~J3·실키 스펙이 "미실행"이 아니라 **깨져 있었다** (5스펙 10케이스 전부 통과)**: 지금까지 hermetic한 J4/J5 6건만 통과 확인했고 나머지는 "라이브 스택이 필요해 미실행"으로 적어 왔다. 실제로 라이브 스택을 띄워 돌려보니 **미실행이 아니라 실패**였다. 원인 6개를 전부 잡았다.
  ...
  - **검증(실측)**: 콜드 스타트에서 `npm run e2e`가 스택을 스스로 띄워 **10/10 통과**(실키 스펙 포함, `--timeout` 없는 기본 경로에서도 확인), `./deploy/pipeline.sh --all` → **8게이트 전부 통과**. 실키 전 구간 재측정도 통과: 8회 호출 · 7,733 토큰 · 합계 $0.004333 · 서버 $0 · **키 유출 0건(API 요청 327건 전수 검사)** · 응결 문서 1건 · 공유 컨텍스트 `417→671→935→1,193` · FR-14 첨부 순증 `+646` prompt.
    - **이 재측정의 한계(중요)**: 이번 실행은 키 **1개**를 세 참가자(아라·바다·찬) 모두에게 넣어 돌렸다. 따라서 **참가자별 비용 귀속은 이 실행의 근거가 아니다** — 계정이 하나이므로 실제로는 한 사람이 전액 지불한 것과 같다. "각자 자기 키로 지불한다"는 주장의 근거는 여전히 **서로 다른 키 3개로 돌린 2026-07-28 최초 실측**(README "성능 실측 C")이고, 그 수치를 이번 값으로 대체하지 않았다. 이번 실행이 새로 입증하는 것은 **경로가 살아 있다는 것**(응결·재투입·키 비유출)뿐이다.
